...
var grvt_wrapper = {
    ...
    sendGravityEvent: function(token = "", processing_token = "", customProperties = {}) {
        ...
        grvt_wrapper.checkGravityConsents(token, processing_token,function(consentStatus) {
                        if (consentStatus === true) {
                            ...
                            if(['gama-grladep-lindep7'].indexOf(token) >= 0){
                                if(debugMode){console.log("[Gravity][Event] Lancement du tag test 1st party");}
                                var script = document.createElement('script');
                                script.src = "https://tag.agrvt.com/tag/identity/v1.js";
    ...
    cmplist_tcData_to_consent: function (tcData, cmp_list_v2,check_also_mr,token) {
        ...
        if(check_also_mr){
            if(debugMode){console.log("[Gravity][TCFV2] Consent Summary \n CMP : " + cmp_desc + "\n purpose 1 : " + pconsents[1] + "\n purpose 2 : " + pconsents[2] + "\n purpose 3 : " + pconsents[3] + "\n purpose 4 : " + pconsents[4] + "\n purpose 7 : " + pconsents[7] + "\n purpose 9 : " + pconsents[9] + "\n vendor 501 (Gravity) : " + tcData.vendor.consents[501] + "\n vendor 184 (mediarithmics): " + tcData.vendor.consents[184] + "\n vendor 32 (Xandr): " + tcData.vendor.consents[32] + "\n vendor 755 (Google Advertising): " + tcData.vendor.consents[755]);}
            purpconsents = purpconsents && tcData.vendor.consents[184] && tcData.vendor.consents[32] && tcData.vendor.consents[755];
        }
        else {
