...
class CleoPlayer {
    ...
    eventsHandling() {
        ...
        /* EVENT : < adend > | Gestion du saut entre la pub et le contenu Live
            Méthode recommandée par Brightcove */
        ...
        /* EVENT : < ads-ad-started > | Début de chaque pub : Affichage (& actualisation) du compteur de pubs */
        this.player.on('ads-ad-started', () => {
            ...
            adCounter.innerText = `Votre programme dans un instant`;
        ...
        /* EVENT : < ads-pod-started > | Début de chaque slot publicitaire : Affichage du bouton de mute de la pub sur Webmobile */
...
function playerBridge() {
    ...
    divNextPlayers.forEach((el, index) => {
        ...
        /* Vérification de la non-existence d'un script doublon avant l'ajout dans l'objet */
    ...
    function bridgeFinished() {
        ...
        /* Condition pour décaler le PIP à droite du menu sur 01net */
        ...
        if (typeof outbrainElement != 'undefined' && outbrainElement != null) {
            window.addEventListener('scroll', throttle(() => {
                ...
                if (isArticle && isMobile.any()) {
                  ...
                  if ((scrollY + 79) > throttleOutbrainElement.offsetTop) {  // On vérifie si le player a dépassé la zone Outbrain
                    if (player.pip().state.active) {
                        player.pip().disable();
                        player.pip().deactivate();
                    }
